function Rec = HybridDecryption(y, Pattern,Compressive_ratio, pixelSize, waveLength,z_d1, z_d2, x_shift, y_shift,theta1,theta2)
vl_setupnn;
mag = Compressive_ratio;
IterNum_initial = 20;                             % initial iteration 
IterNum_GAP = 20;                                  % GAP iteration  
lambda_gap = 1e-5;                                % weight coefficient 
...
imInitial_objpad=imresize(sqrt(mean(y,3)),[imSize*mag imSize*mag]);
Rec = Backward_Exp(y, Pattern,Compressive_ratio, pixelSize, waveLength,z_d1, z_d2, x_shift, y_shift,imInitial_objpad, IterNum_initial);
...
for isig = 1:IterNum_GAP   
    ...
    yb = imresize(yb,[imSize,imSize]);
	y_new = (y-lambda_gap*yb);
    Rec_new = Backward_Exp(y_new, Pattern, Compressive_ratio, pixelSize, waveLength,z_d1, z_d2, x_shift, y_shift, Rec, 1);
